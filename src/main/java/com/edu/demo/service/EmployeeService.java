...
import com.edu.demo.entity.EmpRole;
...
import com.edu.demo.repository.DepartmentRepo;
import com.edu.demo.repository.EmployeeRepo;
...
if(d==null)
	...
	emp.setEmpPassword(encoder.encode(emp.getEmpAddress()));
...
public Employee getOne(int empId) throws EmployeeNotFoundException {
	if(!repo.existsById(empId))throw new EmployeeNotFoundException("no employee found");
	else {
...
public Employee getByJoiningDate(LocalDate date) throws EmployeeNotFoundException {
	if(!repo.existsById(empId))throw new EmployeeNotFoundException("no employee found");
	else {
		return repo.findById(empId).get();
